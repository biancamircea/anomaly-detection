...
def get_sim_kendall(ind, ind_scores, model_list, n_sim, n_datasets, ap_values):
    ...
    # meta_ap = ap_values[meta_train_list, :]
    kendall_sim = []
    for i in range(n_datasets):
        if i != ind:
            # kendall_sim.append(kendalltau(ind_scores, ap_values[i, model_list])[0])
...
def get_qth(n_hp_configs, sorted_ap_val, our_result, verbose=False):
    ...
    for i in range(n_hp_configs):
        if verbose:
            print('top', i+1,
                  wilcoxon(sorted_ap_val[:, n_hp_configs-i-1], our_result, alternative='greater'), np.mean(sorted_ap_val[:, n_hp_configs-i-1]),
                  np.mean(our_result))
